...
type Condition = {
  title: string;
  category: string;   // e.g. "Property Insurance"
  code: string;       // e.g. "CNDT-71284"
  dueDate: string;    // dd-mm-yyyy (as shown in the mocks)
  note: string;
};
    ...
    conditions: [
      {
        ...
        dueDate: '09-07-2025',
        note:
          'If borrower entity is submitted on monthly rent. The cut-off date for submission of statement is 15th of every month'
      },
      {
        title: 'Condition text',
...
export default function Overdue() {
  ...
  // ensure scrolling not locked by other views
  ...
  const chips = useMemo(() => {
    ...
    if (overdueRange !== 'all') {
      out.push(
        overdueRange === 'lt7' ? 'Less than 7 Days'
        : overdueRange === 'lt14' ? 'Less than 14 Days'
        : '7–14 Days'
      );
    }
  ...
  // filtered rows (filter conditions inside each card)
  const filteredRows = useMemo(() => {
    ...
    return rows
      .map((r) => {
        const passText =
          !q || r.name.toLowerCase().includes(q) || r.ctpt.toLowerCase().includes(q);
  ...
  return (
    <>
      <AppHeader title="Overdue" onBack={() => history.replace('/dashboard')} />

      {/* IonContent provides native scrolling + safe padding */}
      <IonContent className="ov-content" fullscreen>
        <div className="ov-wrap">
          <div className="ov-top">
            <div className="seg" role="tablist" aria-label="Root or Items">
              <button
                className={`seg-btn ${tab === 'root' ? 'active' : ''}`}
                onClick={() => setTab('root')}
                role="tab"
              ...
              <button
                className={`seg-btn ${tab === 'items' ? 'active' : ''}`}
            ...
            <div className="ov-controls">
              <div className="search" role="search">
                ...
                <input
                  ...
                  placeholder="Search"
                  aria-label="Search by customer or CTPT"
                />
              </div>

              <button
                className="filter-pill"
                onClick={() => { setShowFilter(true); setFilterTab('category'); }}
          ...
          <div className="ov-list">
                      ...
                      {row.conditions.map((c, idx) => (
                        <div className="cond" key={idx}>
                          <div className="cond-grid">
                            <div className="kv">
                              <span className="k">Condition Category</span>
                              <span className="v">{c.category}</span>
                            </div>
                            <div className="kv">
                              <span className="k">CNDT ID</span>
                              <span className="v v--mono">{c.code}</span>
                            </div>
                            <div className="kv">
                              <span className="k">Due Date</span>
                              <span className="v v--due">{c.dueDate}</span>
                            </div>
                          </div>
            ...
            {filteredRows.length === 0 && (
              <div className="empty-card">No results. Adjust filters or search.</div>
            )}
          </div>
      ...
      {/* FILTER DIALOG */}
      {showFilter && (
        <div className="f-overlay" role="dialog" aria-modal>
          <div className="f-panel" role="document">
            ...
            <div className="f-body">
              <div className="f-rail" role="tablist" aria-label="Filter sections">
                <button
                  className={`f-rail-item ${filterTab === 'category' ? 'active' : ''}`}
                  onClick={() => setFilterTab('category')}
                  role="tab"
                  aria-selected={filterTab === 'category'}
                >
                  Condition<br/>Category
                </button>
                <button
                  className={`f-rail-item ${filterTab === 'overdue' ? 'active' : ''}`}
                ...
                <button
                  className={`f-rail-item ${filterTab === 'pending' ? 'active' : ''}`}
                    ...
                    <div className="f-row between">
                      <label className="f-check">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) setSelectedCategories([...ALL_CATEGORIES]);
                  ...
                  <div className="f-block">
                    <div className="f-radio-col">
                      <label className="f-radio">
                        <input type="radio" name="ov" checked={overdueRange === 'lt7'} onChange={() => setOverdueRange('lt7')} />
                        <span>Less than 7 Days</span>
                      </label>
                      <label className="f-radio">
                        <input type="radio" name="ov" checked={overdueRange === 'lt14'} onChange={() => setOverdueRange('lt14')} />
                        <span>Less than 14 Days</span>
                      </label>
                      <label className="f-radio">
                        <input type="radio" name="ov" checked={overdueRange === '7to14'} onChange={() => setOverdueRange('7to14')} />
                        <span>7–14 Days</span>
                      </label>
                      <label className="f-radio">
                        <input type="radio" name="ov" checked={overdueRange === 'all'} onChange={() => setOverdueRange('all')} />
                        <span>All</span>
                      </label>
                    </div>
                  </div>
